...
build_deb() {
    echo "[deb] Building .deb (Debian/Ubuntu)..."
    ...
    echo "  -> $(ls "$OUT_DIR"/*"$VERSION"*.deb 2>/dev/null | xargs -n1 basename)"
    echo ""
}

build_rpm() {
    echo "[rpm] Building .rpm (Fedora)..."
    docker build -t motu-mk5-rpm \
        --build-arg VERSION="$VERSION" \
        -f "$SCRIPT_DIR/docker/Dockerfile.fedora" "$PROJECT_DIR"
    docker run --rm -v "$OUT_DIR:/host" motu-mk5-rpm sh -c "cp /out/*.rpm /host/"
    echo "  -> $(ls "$OUT_DIR"/*"$VERSION"*.rpm 2>/dev/null | xargs -n1 basename)"
    echo ""
}

build_arch() {
    echo "[arch] Building .pkg.tar.zst (Arch)..."
    docker build -t motu-mk5-arch \
        --build-arg VERSION="$VERSION" \
        -f "$SCRIPT_DIR/docker/Dockerfile.arch" "$PROJECT_DIR"
    docker run --rm -v "$OUT_DIR:/host" motu-mk5-arch sh -c "cp /out/*.pkg.tar.zst /host/"
    echo "  -> $(ls "$OUT_DIR"/*"$VERSION"*.pkg.tar.zst 2>/dev/null | xargs -n1 basename)"
    echo ""
}
