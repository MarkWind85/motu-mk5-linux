...
- **Fix device control connection resetting every few seconds after reconnect**: On each reconnect the daemon wrote all 2375 writable saved properties to the device in one unpaced burst, and the mk5 closed the control connection roughly two seconds later — 13 consecutive times in one observed session, with no exceptions. Every reset triggered another reconnect and another burst, so device control never held longer than about eight seconds, and audio stopped shortly after the device came up. The burst was redundant as well as harmful: `sync_from_device()` fills the in-memory state with the device's own readings before `restore_to_device()` runs, so the daemon was writing the device's current state back at it. Restore now compares the on-disk state as it was at connect — captured before the sync overwrites it — against what the device reported, and writes only the properties that actually differ, paced 20ms apart. Reconnecting to an unchanged device writes nothing at all. A restore of more than 256 differing properties is refused with a warning, since a diff that large means the saved state no longer describes the attached device.

- **Output trim is left to the hardware (#4)**: `main_trim` and `output_trim` encode output attenuation in dB, where 0–100 maps to 0 down to −100 dB. Writing a saved value back on every reconnect moved the hardware monitoring level with nothing on screen to account for the change. Both properties are now excluded from restore and the device keeps the trim it holds.

## 0.5.3

- **Native WirePlumber 0.5 configuration — fixes slow or missing pro-audio profile at boot**: On WirePlumber 0.5+ (current Arch, Fedora 40+, Pop!_OS), the shipped Lua rule was only loaded through WirePlumber's backward-compatibility shim (`Old configuration file detected` in the journal), which applies the pro-audio profile late and with no timing guarantee. At boot this regularly left the MOTU without its virtual sinks for 45–60 seconds while the daemon's ALSA discovery retries ran down — and when the profile applied after the last retry (or, as observed on device hotplug, not at all), audio routing stayed down until a manual `pactl set-card-profile` + daemon restart. The same rules now also ship as native SPA-JSON at `/usr/share/wireplumber/wireplumber.conf.d/51-motu-mk5.conf`, which WirePlumber 0.5 applies promptly during normal init — at boot and on hotplug. The Lua file is still installed for distros on WirePlumber 0.4; on 0.5 the compat shim loads it redundantly with identical properties, which is harmless (the deprecation warning in the journal remains, and is cosmetic).

- **Fix daemon lock-up while the device streams meter data — sync, shutdown, and control were all affected**: The mk5 pushes property updates over its WebSocket near-continuously whenever signal is present. Three read loops in the daemon (initial state sync, the incoming-update drain, and by extension shutdown handling) were written to "read until the stream goes quiet" — with a streaming device they simply never finished. In a 26-hour journal sample, 13 initial syncs were attempted and exactly 1 completed. Consequences fixed: state sync now completes within a 5-second deadline instead of hanging indefinitely; incoming updates are drained in bounded 100ms batches so the main loop keeps running (router supervision, state saves); and SIGTERM now stops the daemon in seconds — previously it could be ignored entirely, which is why package upgrades saw systemd time out and SIGKILL the daemon.

- **Daemon now recovers when the device appears after startup**: ALSA node discovery used to be a one-shot sequence of 10 retries (~2 minutes) at daemon startup — if the MOTU enumerated on USB after that window (slow firmware boot after power-on, device switched on after the PC, USB re-enumeration), audio routing stayed dead until a manual `systemctl --user restart motu-mk5d`, even though WebSocket device control reconnected fine. After the initial retry envelope the daemon now keeps checking for the MOTU's ALSA nodes every 30 seconds (silently — no journal spam) and starts the audio router as soon as they appear. Observed in practice: a cold-booted mk5 can take over 2 minutes to show up on the USB bus, landing just outside the old retry window.
